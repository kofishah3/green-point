                        ...
                        content={
                          <div className="flex-1 flex flex-col gap-4">
                            {recommendations
                              ...
                              .map((rec, idx, arr) => {
                                // Find matching barangay data for context
                                const barangayData = geoData?.find(b => b.name.toLowerCase() === rec.barangay_name.toLowerCase());

                                const explanation = (() => {
                                  // Build comprehensive explanation
                                  let exp = `${rec.explanation || ''}\n\n`;

                                  if (barangayData) {
                                    // Add barangay-specific context
                                    exp += `**Barangay ${rec.barangay_name} Analysis:**\n\n`;

                                    // Environmental conditions
                                    exp += `**Current Environmental Conditions:**\n`;
                                    exp += `• Greenery Index: ${barangayData.greeneryIndex.toFixed(2)} - `;
                                    if (barangayData.greeneryIndex < 0.3) exp += 'Low vegetation coverage requiring urgent greening intervention.\n';
                                    else if (barangayData.greeneryIndex < 0.5) exp += 'Moderate vegetation coverage with room for improvement.\n';
                                    else exp += 'Good vegetation coverage that can be further optimized.\n';

                                    exp += `• Land Surface Temperature: ${barangayData.lst.toFixed(2)}°C - `;
                                    if (barangayData.lst > 30) exp += 'High surface temperatures indicating significant heat island effect.\n';
                                    else if (barangayData.lst > 27) exp += 'Moderate heat levels that can benefit from cooling interventions.\n';
                                    else exp += 'Relatively lower temperatures compared to city average.\n';

                                    exp += `• Tree Canopy Cover: ${barangayData.treeCanopy.toFixed(2)}% - `;
                                    if (barangayData.treeCanopy < 15) exp += 'Low tree coverage requiring increased urban forestry efforts.\n';
                                    else if (barangayData.treeCanopy < 25) exp += 'Moderate canopy that needs expansion for better cooling.\n';
                                    else exp += 'Good tree coverage providing natural cooling benefits.\n';

                                    exp += `• NDVI: ${barangayData.ndvi.toFixed(2)} - `;
                                    if (barangayData.ndvi < 0.3) exp += 'Sparse vegetation density limiting ecosystem services.\n\n';
                                    else if (barangayData.ndvi < 0.5) exp += 'Moderate vegetation health with growth potential.\n\n';
                                    else exp += 'Healthy vegetation providing environmental benefits.\n\n';
                                  }

                                  // Why this intervention
                                  exp += `**Why This Intervention Works Here:**\n\n`;

                                  const interventionType = rec.intervention_type.toLowerCase();
                                  if (interventionType.includes('tree')) {
                                    exp += `Street trees are highly effective because they provide direct cooling through shade, reduce ambient temperature, and improve air quality along pedestrian corridors. They can reduce surface temperatures significantly while enhancing the neighborhood's aesthetic appeal and property values.\n\n`;
                                  } else if (interventionType.includes('park')) {
                                    exp += `Pocket parks serve as green oases in urban areas, providing multiple ecosystem services. This creates accessible green space for the community while addressing low greenery indices. Parks combine vegetation, permeable surfaces, and open space to maximize cooling and stormwater management.\n\n`;
                                  } else if (interventionType.includes('roof')) {
                                    exp += `Green roofs are particularly effective in areas with limited ground space and high surface temperatures. They insulate buildings, reduce energy costs, and provide additional green space without competing for scarce land.\n\n`;
                                  } else if (interventionType.includes('vertical') || interventionType.includes('wall')) {
                                    exp += `Vertical gardens maximize greening potential in space-constrained areas. This intervention utilizes vertical surfaces to increase vegetation coverage without requiring valuable ground space, while providing cooling and aesthetic benefits to building facades.\n\n`;
                                  } else if (interventionType.includes('rain') || interventionType.includes('garden')) {
                                    exp += `Rain gardens address stormwater management through bio-retention, particularly important for flood-prone areas. These gardens capture runoff, filter pollutants, and recharge groundwater while adding green infrastructure that supports biodiversity and cooling.\n\n`;
                                  } else if (interventionType.includes('forest')) {
                                    exp += `Urban forests provide the highest density of environmental benefits including maximum cooling, carbon sequestration, and habitat creation. This intervention establishes a significant green corridor that reduces the heat island effect and improves overall ecological health.\n\n`;
                                  }

                                  // Expected benefits
                                  exp += `**Expected Environmental Benefits:**\n\n`;
                                  if (rec.cooling_potential) {
                                    exp += `• **Temperature Reduction:** Up to ${rec.cooling_potential.toFixed(2)}°C decrease in ambient and surface temperatures, making outdoor spaces more comfortable and reducing energy demands for cooling.\n\n`;
                                  }
                                  if (rec.canopy_gain) {
                                    exp += `• **Canopy Coverage Increase:** Expected to increase tree canopy coverage by approximately ${rec.canopy_gain.toFixed(1)}%, enhancing shade provision, biodiversity habitat, and urban forest connectivity.\n\n`;
                                  }
                                  if (rec.stormwater_retention) {
                                    exp += `• **Stormwater Management:** Capable of retaining approximately ${Math.round(rec.stormwater_retention)}mm of rainfall, reducing flood risk and preventing overflow of drainage systems during heavy precipitation events.\n\n`;
                                  }
                                  if (rec.pm25_removal) {
                                    exp += `• **PM2.5 Reduction:** Expected to reduce PM2.5 particulate matter by ${rec.pm25_removal.toFixed(2)} µg/m³, contributing to cleaner air and reduced respiratory health risks for residents.\n\n`;
                                  }
                                  if (rec.no2_removal) {
                                    exp += `• **NO2 Reduction:** Expected to reduce nitrogen dioxide levels by ${rec.no2_removal.toFixed(2)} µg/m³, improving air quality and reducing harmful pollutants from vehicle emissions.\n\n`;
                                  }

                                  // Implementation considerations
                                  exp += `**Implementation Considerations:**\n\n`;
                                  exp += `This intervention has been prioritized based on comprehensive analysis of the location's environmental metrics, spatial constraints, and community needs. `;
                                  exp += `The estimated efficiency score of ${rec.efficiency_score?.toFixed(0)}% reflects the optimal balance between cost-effectiveness and environmental impact. `;
                                  exp += `Implementation should consider local site conditions, maintenance requirements, and community engagement to maximize long-term success and sustainability.`;
                                ...
                                return (
                                  <div key={idx}>
                                    <GreenSolutionCard
                                      ...
                                      coolingPotential={rec.cooling_potential}
                                      stormwaterRetention={rec.stormwater_retention}
                                      pm25Removal={rec.pm25_removal}
                                      // Add missing ones
                                      canopyGain={rec.canopy_gain}
                                      no2Removal={rec.no2_removal}
                                    />
                                    {idx < arr.length - 1 && (
