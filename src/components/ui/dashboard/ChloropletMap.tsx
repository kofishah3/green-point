...
export default function MandaueMap({ settings = true }: MandaueMapProps) {
  ...
  useEffect(() => {
    async function loadMapData() {
      try {
        const [boundaries, apiData, recommendations] = await Promise.all([
          fetch('/geo/mandaue_barangay_boundaries.json').then(res => res.json()),
          api.getFinalResults(),
          api.getRecommendations(),
        ]);
        
        const mergedData = mergeAPIData(boundaries, apiData, recommendations);
    ...
    if (feature.properties && feature.properties.name) {
      ...
      layer.on("click", () => {
        ...
        setSelectedBarangay({
          ...
          currentInterventionDetails: feature.properties.current_intervention_details
        });
